...
assistant.add_extractor = function(construct_entities, position, direction, mirrored)
...
local try_replace_pipes_with_tunnels = function(construction_plan, pipe_positions, tunnel_positions, toolbox)
    ...
    for i, tunnel in pairs(tunnels) do
        if not previous_tunnel then
            ...
            if plib.position.taxicab_distance(previous_tunnel.down.position, tunnel.up.position) <= tunnel_length_max then
    ...
    for _, tunnel in pairs(tunnels) do
        ...
        local tunnel_length = plib.position.taxicab_distance(tunnel.down.position, tunnel.up.position)

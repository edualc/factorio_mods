...
- Added safe-zone overlay toggle button (lamp icon, top-right mod-GUI bar): visualises both spawn-suppression rules from `is_safe_spawn` simultaneously — green circles (radius 12) around every powered lamp, and cyan circles (radius 16) around concrete tile areas, one circle per occupied 8×8 tile cell. Always available, no technology required. Refreshes every 5 seconds. Helps plan lamp and floor coverage before nightfall

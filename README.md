...
- Fixed rpg_fireaball and rpg_hadouken splash damage hitting the caster's own robots (construction/logistic/combat): the area-damage trigger now uses `force = "enemy"` so only forces that would attack the caster are hit, the same mechanism vanilla area weapons use to avoid friendly fire.

...
**Reverted to v2.0.2 baseline:** v2.0.3–v2.0.7 (horde marching/sweep improvements, corpse/laser tweaks, the zip/folder-labelling fix, and a `factory_reference` cache desync fix) were rolled back — v2.0.3's unconditional 15-minute sweep and `/zomtorio-sweep` called `factory_reference` regardless of horde activity, which exposed a pre-existing module-local-cache desync (`factory_cache_*` in `horde.lua`, same root cause as the already-fixed `fold_cache`) far more often than v2.0.2 ever did, making coop unplayable. v2.0.2 is the last version confirmed stable in multiplayer.

...
- Only physical damage (gun turrets, player melee) leaves a corpse that can reanimate; all other damage types (fire, laser, explosion, electric, poison, and any unknown/modded type) destroy the zombie utterly — whitelist replaces the old blacklist so new damage types default to no-corpse

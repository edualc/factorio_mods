...
- Fixed horde units spawning but not attacking: `group.start_moving()` was called on an empty unit group before members were added; Factorio rejects `add_member` on a started group, so every entity spawned without a march command and stood idle. Members are now added first, then `set_command` + `start_moving` is called

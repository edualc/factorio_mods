...
- Added safe-zone overlay toggle button (zombie silhouette icon, top-right mod-GUI bar): visualises both spawn-suppression rules from `is_safe_spawn` simultaneously — green outline circles (radius 12) around every powered lamp, and cyan outline circles (radius 16) around boundary cells of concrete tile areas. No filled circles; only outlines so overlapping zones don't stack to an opaque white. Refreshes automatically every 5 seconds or when the player moves more than 50 tiles — always centred on the current position. Icon: Delapouite via game-icons.net, CC BY 3.0
- Robots (logistic and construction) now take infection damage at 50% rate — partial resistance consistent with their full fire immunity (CustomRobotsResistFire). They can still be infected and will eventually die without repair, but last roughly twice as long: `is_safe_spawn` previously checked only the exact ring point, but `find_non_colliding_position` can move the actual spawn up to 16 tiles away. Now uses `find_tiles_filtered` with a 16-tile buffer so any concrete within drift range suppresses the spawn

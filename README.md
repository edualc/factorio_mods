...
- Added `LV_Infection_Resistance` attribute: each level slows CustomZomboid's player infection DoT by the configured bonus (default 5% per level, up to 20 levels = +100% longer survival). The multiplier is exposed via `remote.call("RPG", "GetInfectionResistanceMultiplier", playername)` so CustomZomboid can apply it without a hard dependency.
...
- Added safe-zone overlay shortcut (`zomtorio-safe-zones`): draws semi-transparent green circles (radius 12 tiles, matching the lamp spawn-suppression radius) around every powered lamp near each player. Always available, no technology required. Refreshes every 5 seconds. Helps plan lamp coverage before nightfall
- Robots (logistic and construction) now take infection damage at 50% rate — partial resistance consistent with their full fire immunity (CustomRobotsResistFire). They can still be infected and will eventually die without repair, but last roughly twice as long
- CustomRPGsystem integration: if `CustomRPGsystem` is active, the player infection DoT is scaled by `remote.call("RPG", "GetInfectionResistanceMultiplier")`, so investing in `LV_Infection_Resistance` meaningfully extends survival time under active infection: `is_safe_spawn` previously checked only the exact ring point, but `find_non_colliding_position` can move the actual spawn up to 16 tiles away. Now uses `find_tiles_filtered` with a 16-tile buffer so any concrete within drift range suppresses the spawn

...
require '__CustomLiborio__/prototypes/scripts/util'
...
script.on_configuration_changed(function(f) 
	...
	if f.mod_changes["CustomHeroTurrets"] == nil or f.mod_changes["CustomHeroTurrets"].old_version == nil then
		if heroturrets.force_configuration_change == true then
			log("Forcing update to " .. game.active_mods["CustomHeroTurrets"])
			f.mod_changes["CustomHeroTurrets"] =
			{
				new_version = game.active_mods["CustomHeroTurrets"],
				old_version = "0.1.1"
			}
		else
			return
	...
	log("control.on_configuration_changed " .. f.mod_changes["CustomHeroTurrets"].old_version .. " -> " .. f.mod_changes["CustomHeroTurrets"].new_version)

	for k=1, #heroturrets.on_configuration_changed do local v = heroturrets.on_configuration_changed[k]
		v(f)
	end

	--[[if f.mod_changes["CustomHeroTurrets"].old_version < "0.1.25" then

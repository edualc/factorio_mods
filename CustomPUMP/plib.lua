...
plib.xy = {
    ...
    each = function(xy_table, action)
        for x, subtable in pairs(xy_table) do
            for y, subject in pairs(subtable) do
                ...
                local did_break = action(subject, position)
                if did_break then return end
            end
        end
    end,
    ...
    get_or_create = function(xy_table, position, default)
        local result = plib.xy.get(xy_table, position)
        if not result then 
            result = {}
            plib.xy.set(xy_table, position, result)
        end
        return result
    end,
    ...
    nearest = function(xy_table, search_position)
        ...
        plib.xy.each(xy_table, function(value, entry_position)
            local d = plib.position.distance(search_position, entry_position)
...
plib.position = {
    ...
    distance_squared = function(p1, p2)
        local dx = p1.x - p2.x
        local dy = p1.y - p2.y
        return dx * dx + dy * dy
    end,
    ...
    distance = function(p1, p2)
        return math.sqrt(plib.position.distance_squared(p1, p2))
    end
}

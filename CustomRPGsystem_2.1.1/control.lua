...
local ZOMTORIO_SWARM_TO_INDIVIDUAL = {
	["zomtorio-swarm-small"]                        = "small-biter",
	["zomtorio-swarm-medium"]                       = "medium-biter",
	["zomtorio-swarm-big"]                          = "big-biter",
	["zomtorio-swarm-spitter-small"]                = "small-spitter",
	["zomtorio-swarm-spitter-medium"]               = "medium-spitter",
	["zomtorio-swarm-spitter-big"]                  = "big-spitter",
	["zomtorio-swarm-small-zomtorio-night"]         = "small-biter",
	["zomtorio-swarm-medium-zomtorio-night"]        = "medium-biter",
	["zomtorio-swarm-big-zomtorio-night"]           = "big-biter",
	["zomtorio-swarm-spitter-small-zomtorio-night"] = "small-spitter",
	["zomtorio-swarm-spitter-medium-zomtorio-night"]= "medium-spitter",
	["zomtorio-swarm-spitter-big-zomtorio-night"]   = "big-spitter",
}
...
local individual = ZOMTORIO_SWARM_TO_INDIVIDUAL[entity.name]
local XP
if individual and prototypes.entity[individual] then
	local single_xp = prototypes.entity[individual].get_max_health()
	local kills = remote.interfaces["CustomZomboid"] and remote.call("CustomZomboid", "get_cluster_kills", entity.unit_number) or 1
	XP = single_xp * (kills or 1)
else
	XP = entity.prototype.get_max_health()
end
local player , plname
...
commands.add_command('customrpg-reset-points', 'Reconstruct XP table and reset all habilities to zero. Players can spent all points again', function(event)
local player = game.players[event.player_index]
...
commands.add_command('customrpg-reset-all', 'Reconstruct XP table and reset everything to zero, as a new game', function(event)
local player = game.players[event.player_index]
if player.admin then
	ResetAll()
	game.print({'xp_reset_altert'})
...
commands.add_command('customrpg-players-list', 'List all players', function(event)
local player = game.players[event.player_index]
ListAll(player)
end)
commands.add_command('customrpg-listXPTable', 'List XP table', function(event)
local player = game.players[event.player_index]
ListXPTable(player)
end)

...
local function create_cluster(surface, pos, pop, tier, force, command, shamblers, kind, horde_member)
  ...
  local rec = { pop = pop, tier = tier, kind = kind, horde_member = horde_member or nil,
                shamblers = math.min(shamblers or 0, pop), shambler_acc = 0, kills = 0 }
...
function swarm.on_entity_damaged(event)
  ...
  rec.kills = (rec.kills or 0) + removed
  ...
  if cap_room() > 0 and character_near(surface, pos) then
    ...
    z.pending_pops[entity.unit_number] = rec.kills
    z.swarm[entity.unit_number] = nil
    -- Credit 1 kill to the attacker before destroying: entity.destroy() and
    -- entity.die() both bypass the engine's internal kill-tracking pathway, so
    -- the attacker's kills counter never increments without this explicit write.
    -- pcall guards against non-turret/vehicle entities that lack a kills property.
    local cause = event.cause
    if cause and cause.valid then
      pcall(function() cause.kills = cause.kills + 1 end)
    end
    entity.die(cause and cause.valid and cause.force or nil, cause)
    if survivors > 0 then
  ...
  if rec.pop <= 0 then
    z.pending_pops[entity.unit_number] = rec.kills
    z.swarm[entity.unit_number] = nil
    -- Credit 1 kill to the attacker before destroying (see burst path above).
    local cause = event.cause
    if cause and cause.valid then
      pcall(function() cause.kills = cause.kills + 1 end)
    end
...
local function forget(un)
  ...
  if z.pending_pops then z.pending_pops[un] = nil end

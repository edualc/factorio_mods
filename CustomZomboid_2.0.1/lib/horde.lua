...
local function is_safe_spawn(surface, pos)
  ...
  for _, lamp in ipairs(lamps) do
    if lamp.valid and lamp.energy > 0 then return true end
  end
  return false
end
...
--- still tracks state, it simply has nowhere to spawn. Skips positions that are
--- paved with concrete or lit by a powered lamp.
local function spawn_near_anchors(surface, count, tier, tick)
  ...
  for _, c in pairs(chars) do
    if c.valid then
      ...
      if not is_safe_spawn(surface, pos) then
